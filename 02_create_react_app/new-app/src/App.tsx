...
function App() {
  ...
  for (let i = 8; i > 0; i--) { items.push(<li key={i}>{i}</li>); }
  ...
  Object.entries(playerKeyValues).forEach(
    ([key, value]) => {
      infoPairs.push(<p>{key}:{value}<br/></p>)
    }
  );
  ...
  const playerArray:JSX.Element[] = []; 
    Object.entries(playerKeyValues).map(([key, value], index) => { 
      playerArray.push(<span key={index}>{key}: {value}</span>) 
    }
  )
